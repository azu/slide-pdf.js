...
prop.fitItSize = function () {
    ...
    return new Promise(function (resolve) {
        ...
        that.canvas.width = containerRect.width;
        that.canvas.height = containerRect.height;
        resolve(containerRect);
    }).then(function () {
            return that.renderPage(that.pageNum);
        });
};
prop.cleanup = function () {
    var range = document.createRange();
    range.selectNodeContents(this.textLayerDiv);
    range.deleteContents();
    range.selectNodeContents(this.annotationLayer);
    range.deleteContents();
};
prop.renderPage = function renderPage(num) {
    ...
    return that.pdfDoc.getPage(num).then(function (page) {
        ...
        Promise.all([renderPromise, textLayerPromise]).then(function (result) {
            console.info(result);
            setupAnnotations(page, viewport, that.annotationLayer);
        });
    });
};
